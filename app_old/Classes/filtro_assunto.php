...
$query_count = $pdo->prepare("SELECT COUNT(*) FROM topicos WHERE id_materias IN (:termo)");
$query_count->bindValue(':termo',  $termo , PDO::PARAM_STR);
$query_count->execute();
$total = $query_count->fetchColumn();

// Query para buscar os registros
$query = $pdo->prepare("SELECT * FROM topicos WHERE id_materias IN (:termo) LIMIT :limit OFFSET :offset");
